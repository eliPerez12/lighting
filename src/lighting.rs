...
pub enum Light {
    Radial {pos: Vector2, color: Vector4, radius: f32},
    Ambient {color: Vector4},
}

impl Light {
    pub fn default_radial() -> Light {
        Light::Radial { pos: Vector2::new(0.0, 0.0), color: Vector4::new(1.0, 1.0, 1.0, 1.0), radius: 350.0}
    }
...
impl LightEngine {
    ...
    pub fn update_shader_values(&self, shader: &mut Shader) {
        shader.set_shader_value_v(
            ...
            self.lights.iter().map(|light| light.1.pos()).collect::<Vec<Vector2>>().as_slice(),
        );
        shader.set_shader_value_v(
            self.shader_uniforms.color,
            self.lights.iter().map(|light| light.1.color()).collect::<Vec<Vector4>>().as_slice(),
        );
        shader.set_shader_value(
            self.shader_uniforms.amount,
            self.lights.len() as i32,
        );
        shader.set_shader_value_v(
            self.shader_uniforms.radius,
            self.lights.iter().map(|light| light.1.radius()).collect::<Vec<f32>>().as_slice(),
        );
        shader.set_shader_value_v(
            self.shader_uniforms.light_type,
            self.lights.iter().map(|light| light.1.light_type()).collect::<Vec<i32>>().as_slice(),
        );
    }
}